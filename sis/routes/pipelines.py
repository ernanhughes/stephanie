...
import yaml
import copy
from sis.utils.data_utils import sanitize_config, get_run_config
...
def pipeline_detail(request: Request, pipeline_id: int):
    ...
    for e in evaluations:
        e["scores"] = [s.to_dict() for s in scores if s.evaluation_id == e["id"]]
...
def pipeline_evaluations(request: Request, pipeline_id: int):
    ...
    print(f"Found {len(evaluations)} evaluations for pipeline run {pipeline_id}")
    logger.info(f"Found {len(evaluations)} evaluations for pipeline run {pipeline_id}")
    # Attach dimension scores for each evaluation
    for eval in evaluations:
        eval.dimension_scores = memory.scores.get_scores_for_evaluation(eval.id)
