...
# Access to memory.bus_events (assert it exists on app.state)
...
def api_recent(
    ...
    subject_like: Optional[str] = Query(None, description="SQL LIKE, e.g. 'stephanie.events.arena.run.%'"),
    event: Optional[str] = Query(None),
    since_id: Optional[int] = Query(None, description="Return rows with id > since_id"),
):
    """
    Return recent bus_events rows. If since_id is given, tail after that id.
    """
    try:
        ...
        return JSONResponse([r.to_dict(include_payload=False, include_extras=False) for r in rows])
    except Exception as e:
        log("api_recent error:", repr(e))
...
def api_event_detail(request: Request, event_id: int):
    """
    Return a single bus_event row with full payload/extras.
    """
    try:
        store = _store(request)
        ...
        return JSONResponse(row.to_dict(include_payload=True, include_extras=True))
    except HTTPException:
        raise
    except Exception as e:
        log("api_event_detail error:", repr(e))
        return JSONResponse({"error": str(e)}, status_code=500)
...
async def stream(
    ...
    async def gen():
        try:
            ...
            err = {"event":"_error","where":"connect","type":type(e).__name__,"msg":str(e),"repr":repr(e),"trace":traceback.format_exc(),"t":time.time()}
        ...
        try:
            ...
            err = {"event":"_error","where":"subscribe","type":type(e).__name__,"msg":str(e),"repr":repr(e),"trace":traceback.format_exc(),"t":time.time()}
        ...
        try:
            ...
            err = {"event":"_error","where":"sse_loop","type":type(e).__name__,"msg":str(e),"repr":repr(e),"trace":traceback.format_exc(),"t":time.time()}
