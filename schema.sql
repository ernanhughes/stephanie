...
CREATE TABLE IF NOT EXISTS experiments (
  ...
  CONSTRAINT uq_experiment_name_domain UNIQUE (name, domain)
);

CREATE INDEX IF NOT EXISTS ix_experiments_name   ON experiments (name);
-- (domain is covered by the UNIQUE above; add a plain index if you need more lookups)
...
CREATE TABLE IF NOT EXISTS experiment_variants (
  ...
  name           VARCHAR(16) NOT NULL,  -- "A" | "B" | "C"...
  is_control     BOOLEAN NOT NULL DEFAULT FALSE,
  payload        JSONB   NOT NULL DEFAULT '{}'::jsonb,
  created_at     TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT uq_variant_name_per_experiment UNIQUE (experiment_id, name)
);

-- helpful lookup index
CREATE INDEX IF NOT EXISTS ix_variants_experiment_id ON experiment_variants (experiment_id);
...
CREATE TABLE IF NOT EXISTS experiment_trials (
  ...
  case_id          INTEGER,                           -- optional link back to your world
  pipeline_run_id  VARCHAR(64),
  domain           VARCHAR(64),

  assigned_at      TIMESTAMP NOT NULL DEFAULT NOW(),
  completed_at     TIMESTAMP,
  performance      DOUBLE PRECISION,                  -- overall score (nullable)
  tokens           INTEGER,
  cost             DOUBLE PRECISION,
  wall_sec         DOUBLE PRECISION,
  meta             JSONB NOT NULL DEFAULT '{}'::jsonb,

  -- ensures deterministic assignment per (experiment, case) at the variant level
  CONSTRAINT uq_trial_variant_case UNIQUE (variant_id, case_id)
);

CREATE INDEX IF NOT EXISTS ix_trials_case_id      ON experiment_trials (case_id);
CREATE INDEX IF NOT EXISTS ix_trials_completed    ON experiment_trials (completed_at);
-- Optional: speed up lookups by variant
CREATE INDEX IF NOT EXISTS ix_trials_variant_id   ON experiment_trials (variant_id);

-- ---------- 4) experiment_trial_metrics ----------
CREATE TABLE IF NOT EXISTS experiment_trial_metrics (
  id         INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  trial_id   INTEGER NOT NULL REFERENCES experiment_trials(id) ON DELETE CASCADE,
  key        VARCHAR(64) NOT NULL,
  value      DOUBLE PRECISION NOT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS ix_trial_metric_key   ON experiment_trial_metrics (key);
CREATE INDEX IF NOT EXISTS ix_trial_metric_trial ON experiment_trial_metrics (trial_id);

-- ---------- 5) experiment_model_snapshots ----------
CREATE TABLE IF NOT EXISTS experiment_model_snapshots (
  id            INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  experiment_id INTEGER NOT NULL REFERENCES experiments(id) ON DELETE CASCADE,
  name          VARCHAR(128) NOT NULL,   -- e.g., "learning_strategy"
  domain        VARCHAR(128),            -- optional scope key
  version       INTEGER NOT NULL,
  payload       JSONB   NOT NULL DEFAULT '{}'::jsonb,  -- serialized knobs/data
  validation    JSONB,                                  -- stats bundle that justified commit
  committed_at  TIMESTAMP NOT NULL DEFAULT NOW(),
  created_at    TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT ix_model_snapshots_unique UNIQUE (experiment_id, name, domain, version)
);

CREATE INDEX IF NOT EXISTS ix_model_snapshots_experiment_id ON experiment_model_snapshots (experiment_id);
CREATE INDEX IF NOT EXISTS ix_model_snapshots_name         ON experiment_model_snapshots (name);
CREATE INDEX IF NOT EXISTS ix_model_snapshots_domain       ON experiment_model_snapshots (domain);
CREATE INDEX IF NOT EXISTS ix_model_snapshots_version      ON experiment_model_snapshots (version);
