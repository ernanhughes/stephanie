...
from stephanie.models.evaluation import EvaluationORM  # Assuming EvaluationORM exists
...
class PlanTraceORM(Base):
    ...
    goal: Mapped[Optional[GoalORM]] = relationship("GoalORM", back_populates="plan_traces") # Add plan_traces to GoalORM
...
class ExecutionStepORM(Base):
    ...
    plan_trace_id: Mapped[int] = mapped_column(Integer, ForeignKey("plan_traces.id"), nullable=False)
    plan_trace: Mapped["PlanTraceORM"] = relationship("PlanTraceORM", back_populates="execution_steps")
    ...
    evaluation_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("evaluations.id"), nullable=True, unique=True)
...
# Note on Schema Creation:
# Remember to create the corresponding database tables.
# If using Alembic, you'll need to generate and run a migration.
# Example command (if set up):
#   alembic revision --autogenerate -m "Add PlanTraceORM and ExecutionStepORM"
#   alembic upgrade head

# Example SQL (PostgreSQL style, adjust for your DB):
# CREATE TABLE IF NOT EXISTS public.plan_traces (
#     id SERIAL PRIMARY KEY,
#     trace_id TEXT NOT NULL UNIQUE,
#     goal_id INTEGER REFERENCES public.goals(id) ON DELETE CASCADE,
#     goal_embedding_id INTEGER, -- REFERENCES public.embeddings(id) -- if linking ORM
#     goal_text TEXT NOT NULL,
#     plan_signature TEXT NOT NULL,
#     final_output_text TEXT NOT NULL,
#     final_output_embedding_id INTEGER, -- REFERENCES public.embeddings(id) -- if linking ORM
...
# );
# CREATE INDEX idx_plan_traces_trace_id ON public.plan_traces (trace_id);
# CREATE INDEX idx_plan_traces_goal_id ON public.plan_traces (goal_id);
# CREATE INDEX idx_plan_traces_created_at ON public.plan_traces (created_at);
#
# CREATE TABLE IF NOT EXISTS public.execution_steps (
#     id SERIAL PRIMARY KEY,
#     plan_trace_id INTEGER NOT NULL REFERENCES public.plan_traces(id) ON DELETE CASCADE,
#     step_order INTEGER NOT NULL,
#     step_id TEXT NOT NULL,
#     description TEXT NOT NULL,
#     output_text TEXT NOT NULL,
#     output_embedding_id INTEGER, -- REFERENCES public.embeddings(id) -- if linking ORM
#     evaluation_id INTEGER UNIQUE REFERENCES public.evaluations(id) ON DELETE SET NULL,
#     meta JSONB,
#     created_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT (now() AT TIME ZONE 'utc')
# );
# CREATE INDEX idx_execution_steps_plan_trace_id ON public.execution_steps (plan_trace_id);
# CREATE INDEX idx_execution_steps_step_order ON public.execution_steps (step_order);
# CREATE INDEX idx_execution_steps_evaluation_id ON public.execution_steps (evaluation_id);