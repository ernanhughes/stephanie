...
class ExperimentORM(Base):
    ...
    name: Mapped[str] = mapped_column(String(128), index=True)  # e.g., "verification_strategy"
    label: Mapped[Optional[str]] = mapped_column(String(128))   # optional human label
    ...
    config: Mapped[Dict[str, Any]] = mapped_column(JSON, default=dict)  # knobs (min_samples, p-value, etc.)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now)

    variants = relationship("VariantORM", back_populates="experiment", cascade="all, delete-orphan")
    model_snapshots = relationship("ExperimentModelSnapshotORM", back_populates="experiment", cascade="all, delete-orphan")
    __table_args__ = (
...
class VariantORM(Base):
    ...
    payload: Mapped[Dict[str, Any]] = mapped_column(JSON, default=dict)  # serialized Strategy, Arena settings, etc.
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now)

    experiment = relationship("ExperimentORM", back_populates="variants")
    trials = relationship("TrialORM", back_populates="variant", cascade="all, delete-orphan")
...
class TrialORM(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    variant_id: Mapped[int] = mapped_column(ForeignKey("experiment_variants.id", ondelete="CASCADE"))
    ...
    performance: Mapped[Optional[float]] = mapped_column(Float)  # e.g., overall score
    ...
    __table_args__ = (
        ...
        UniqueConstraint("variant_id", "case_id", name="uq_trial_variant_case"),
...
class TrialMetricORM(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    trial_id: Mapped[int] = mapped_column(ForeignKey("experiment_trials.id", ondelete="CASCADE"))
    key: Mapped[str] = mapped_column(String(64))      # e.g., "overall", "k", "g", "avg_gain"
    value: Mapped[float] = mapped_column(Float)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now)

    __table_args__ = (
        Index("ix_trial_metric_key", "key"),
    )
...
class ExperimentModelSnapshotORM(Base):
    ...
    name = Column(String(128), nullable=False, index=True)   # e.g., "learning_strategy"
    domain = Column(String(128), nullable=True, index=True)  # optional scope key
    ...
    validation = Column(JSON, nullable=True)                 # stats bundle that justified commit
    committed_at = Column(DateTime, nullable=False, default=datetime.now)
    created_at = Column(DateTime, nullable=False, default=datetime.now)

    experiment = relationship("ExperimentORM", back_populates="model_snapshots")

    __table_args__ = (
        Index("ix_model_snapshots_unique", "experiment_id", "name", "domain", "version", unique=True),
    )
