...
class ScorableORM(Base):
    __tablename__ = "scorables"
    ...
    source_scorable_id = Column(Integer, ForeignKey("scorables.id"))
    strategy = Column(String)
    confidence = Column(Float, default=0.0)
    rational = Column(String)
    ...
    # Relationships
    goal = relationship("GoalORM", back_populates="scorables")
    prompt = relationship("PromptORM", back_populates="scorables")
    source_scorable = relationship("ScorableORM", remote_side=[id])
    pipeline_run = relationship("PipelineRunORM", back_populates="scorables")
