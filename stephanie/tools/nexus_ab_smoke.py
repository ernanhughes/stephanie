...
def latest_nexus_report_under(runs_root: Path):
    ...
    cand = sorted(
        runs_root.glob("run-*/nexus_improver_report.json"),
...
METRICS = [
    ...
    (
        "sicql.clarity.uncertainty",
        "sicql.clarity.attr.uncertainty",
        "down",
        "mean",
    ),
    (
        "sicql.coverage.uncertainty",
        "sicql.coverage.attr.uncertainty",
        "down",
        "mean",
    ),
    (
        "sicql.faithfulness.uncertainty",
        "sicql.faithfulness.attr.uncertainty",
        "down",
        "mean",
    ),
    (
        "sicql.clarity.advantage",
        "sicql.clarity.attr.advantage",
        "up",
        "mean",
    ),  # less negative → “up”
    (
        "sicql.coverage.advantage",
        "sicql.coverage.attr.advantage",
        "up",
        "mean",
    ),
    (
        "sicql.faithfulness.advantage",
        "sicql.faithfulness.attr.advantage",
        "up",
        "mean",
    ),
]
...
def augment_with_features(
    run_arm_dir: Path, metrics_dict: dict, wanted=METRICS
):
    """
...
def main():
    ...
    ap.add_argument(
        "--strict",
        action="store_true",
        help="non-zero exit if any guard fails",
    )
    ...
    ap.add_argument(
        "--nexus-a",
        help="Path to baseline NexusImprover report or its run dir (runs/run-*)",
    )
    ap.add_argument(
        "--nexus-b",
        help="Path to targeted NexusImprover report or its run dir (runs/run-*)",
    )
    ap.add_argument(
        "--nexus-autodiscover",
        action="store_true",
    ...
    if not base_metrics and not targ_metrics:
        print(
            colorize(
                "Missing run_metrics.json for both arms. Check --root and run_id.",
                "r",
            )
        )
        sys.exit(2)
    ...
    for name, path, direction, stat in METRICS:
        ...
        print(
            f"{name:28s}  base={base_s:>8s}  targ={targ_s:>8s}  Δ%={delta_s}"
        )
    ...
    # Length blowup
    b_len = dot_get(base_metrics, "text.len")
    t_len = dot_get(targ_metrics, "text.len")
    if b_len is not None and t_len is not None and b_len > EPS:
        ratio = t_len / max(EPS, b_len)
        guard(
            ratio <= args.warn_length,
    ...
    if s_base is not None and s_targ is not None:
        if s_base >= 99.0 and s_targ >= 99.0:
            print(
                colorize(
                    "• Note: sicql.aggregate appears saturated (~100).", "y"
                )
            )
        else:
    ...
    b_knn, t_knn = (
        dot_get(base_metrics, "mutual_knn_frac"),
        dot_get(targ_metrics, "mutual_knn_frac"),
    )
    if b_knn is not None and t_knn is not None and abs(b_knn) > EPS:
        knn_rel = (t_knn - b_knn) / abs(b_knn)
        guard(
            knn_rel >= -args.knn_drop,
    ...
    b_cc, t_cc = (
        dot_get(base_metrics, "clustering_coeff"),
        dot_get(targ_metrics, "clustering_coeff"),
    )
    if b_cc is not None and t_cc is not None and abs(b_cc) > EPS:
        cc_rel = (t_cc - b_cc) / abs(b_cc)
        guard(
            cc_rel >= -args.cluster_drop,
    ...
    b_sp, t_sp = (
        dot_get(base_metrics, "spatial.mean_edge_len"),
        dot_get(targ_metrics, "spatial.mean_edge_len"),
    )
    ...
    if args.fail_on_zero_ga:
        ga0 = (
            dot_get(base_metrics, "goal_alignment.mean") in (0, 0.0)
            and dot_get(base_metrics, "goal_alignment.p90") in (0, 0.0)
            and dot_get(targ_metrics, "goal_alignment.mean") in (0, 0.0)
            and dot_get(targ_metrics, "goal_alignment.p90") in (0, 0.0)
        )
        guard(
            not ga0,
            "Goal alignment not zeroed.",
            "Goal alignment both zero (base & targ).",
        )
    ...
    if args.nexus_autodiscover and (nexus_a is None or nexus_b is None):
        nr = Path("runs")
        newest = sorted(
            nr.glob("run-*/nexus_improver_report.json"),
            key=lambda p: p.stat().st_mtime,
            reverse=True,
        )
    ...
    if nexus_a or nexus_b:
        ...
        def line(label, key, nd=4):
            ...
            print(
                f"{label:22s}  A={fmt(va, nd):>8s}  B={fmt(vb, nd):>8s}  Δ%={rels}"
            )
        ...
        if sa and sb:
            ...
            if (
                ga_base is not None
                and ga_targ is not None
                and (ga_targ - ga_base) > 0.05
            ):
    ...
    if args.csv_out:
        with open(args.csv_out, "w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(
                f, fieldnames=["name", "path", "base", "targ", "rel_impr"]
            )
            w.writeheader()
            for r in rows:
                w.writerow(r)
    ...
    if args.md_out:
        ...
        if nexus_a or nexus_b:
            ...
            def mline(lbl, key):
                ...
                md.append(
                    f"- **{lbl}**: A={fmt(sa.get(key))} → B={fmt(sb.get(key))} (Δ%={fmt(None if relv is None else relv * 100, 1)})"
                )

            for k, lbl in [
                ("win_rate", "Win rate"),
                ("mean_lift", "Mean lift"),
                ("topk_lift", "Top-K lift"),
                ("novelty_retention", "Novelty retention"),
                ("diversity_raw", "Diversity (pre)"),
                ("diversity_post", "Diversity (post)"),
            ]:
                mline(lbl, k)
