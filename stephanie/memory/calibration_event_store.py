...
class CalibrationEventStore(BaseSQLAlchemyStore):
    ...
    def __init__(
        self, session_maker, logger=None, model_dir: str = "data/calibration"
    ):
    ...
    def add(
        self, event: Union[CalibrationEventORM, Mapping]
    ) -> CalibrationEventORM:
        def op(s):
            if not isinstance(event, CalibrationEventORM):
                ...
                scorable_type = _as_str(
                    payload.get("scorable_type"), "unknown"
                )
                entity_type_raw = payload.get("entity_type")
                entity_type = (
                    _as_str(entity_type_raw)
                ...
                event_obj = CalibrationEventORM(
                    ...
                    **(
                        {"features": features}
                        if "features" in CalibrationEventORM.__table__.columns
                        else {}
                    ),
                )
            else:
    ...
    def count_by_domain(self, domain: str) -> dict:
        def op(s):
            pos_expr = func.coalesce(
                func.sum(
                    case(
                        (CalibrationEventORM.is_relevant.is_(True), 1), else_=0
                    )
                ),
                0,
            )
            neg_expr = func.coalesce(
                func.sum(
                    case(
                        (CalibrationEventORM.is_relevant.is_(False), 1),
                        else_=0,
                    )
                ),
                0,
    ...
    def fetch_events(
        self, domain: str, limit: int = 10000, require_features: bool = False
    ) -> List[Dict]:
        def op(s):
            rows = (
                s.query(CalibrationEventORM)
            ...
            for r in rows:
                ...
                if not isinstance(feats, dict):
                    feats = {
                        ...
                        "correctness": float(
                            getattr(r, "correctness", 0.0) or 0.0
                        ),
                        "coherence": float(
                            getattr(r, "coherence", 0.0) or 0.0
                        ),
                        "citation_support": float(
                            getattr(r, "citation_support", 0.0) or 0.0
                        ),
                    }
    ...
    def save_calibrator(self, domain: str, model: dict) -> None:
        ...
        try:
            ...
            if self.logger:
                self.logger.log(
                    "CalibrationSaved", {"domain": domain, "path": path}
                )
        except Exception as e:
            if self.logger:
                self.logger.log(
