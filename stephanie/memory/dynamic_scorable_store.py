...
class DynamicScorableStore(BaseSQLAlchemyStore):
    ...
    def get_by_run(self, run_id: str) -> List[DynamicScorableORM]:
        return self._run(
            lambda s: s.query(DynamicScorableORM).filter_by(run_id=run_id).all(),
        )

    def get_by_pipeline_run(self, pipeline_run_id: str) -> List[DynamicScorableORM]:
        return self._run(
            lambda s: s.query(DynamicScorableORM)
    ...
    def get_by_case(self, case_id: int) -> List[DynamicScorableORM]:
        return self._run(
            lambda s: s.query(DynamicScorableORM).filter_by(case_id=case_id).all(),
        )
        ...
        *,
        ...
        return self._run(
            lambda s: s.query(DynamicScorableORM)
                       ...
                       .order_by(desc(DynamicScorableORM.created_at),
                                 desc(DynamicScorableORM.id))
                       .first(),
        )
