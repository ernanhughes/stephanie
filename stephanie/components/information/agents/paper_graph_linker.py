...
from stephanie.components.information.graph.paper_graph_abi import (
    GraphEdge, GraphNode, PaperGraphABI)
...
from stephanie.components.information.linkers.base import (BaseSectionLinker,
                                                           section_pid)
from stephanie.components.information.linkers.citation import CitationLinker
from stephanie.components.information.linkers.knn import SemanticKNNLinker
from stephanie.components.information.linkers.overlap import \
    EntityOverlapLinker
...
class PaperGraphLinkerAgent(BaseAgent):
    ...
    async def run(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        root_sections = [s for s in all_sections if section_pid(s) == root_arxiv_id]
        corpus_sections = [s for s in all_sections if section_pid(s) != root_arxiv_id]
        ...
        log.info("GraphLinker: root=%d corpus=%d total=%d", len(root_sections), len(corpus_sections), len(all_sections))
