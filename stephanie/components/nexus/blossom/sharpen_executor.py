...
class BlossomSharpenExecutorAgent(BaseAgent):
    ...
    async def run(self, context: Dict[str, Any]) -> Dict[str, Any]:
        self.report({"event": "start", "step": "BlossomExecutor", "details": "GoT/ToT blossom with sharpening"})
        ...
        for doc in docs:
            ...
            if not doc_id:
                self.logger.log("BlossomSkipNoDocId", {"doc_keys": list(doc.keys())})
                continue
            ...
            baseline_summary, baseline_obj = self._fetch_baseline_summary(doc_id, doc)
            if not baseline_summary:
                self.logger.log("BlossomBaselineMissing", {"doc_id": doc_id})
                self.memory.blossoms.update_status(blossom.id, "failed")
                continue

            base_metrics = self._score_summary(baseline_summary, self._fetch_abstract(doc.get("id") or doc_id), doc.get("summary", ""))
            ...
            root = self.memory.blossoms.add_node({
                ...
                "accepted": True,
                "scores": {"overall": base_metrics.get("overall", 0.0), "metrics": base_metrics},
                "features": None,
                "tags": ["root", "baseline"],
                "extra_data": {"doc_id": str(doc_id), "title": doc.get("title", "")},
            })
            ...
            while frontier:
                ...
                candidates = self._expand_and_sharpen(
                    doc=doc,
                    baseline_text=parent.sharpened_text or parent.state_text or "",
                    k=self.max_branching
                )

                # Rank candidates by overall score; pick top-k (already limited by k)
                candidates.sort(key=lambda c: c["metrics"]["overall"], reverse=True)

                for order_idx, cand in enumerate(candidates):
                    gain = cand["metrics"]["overall"] - parent_overall
                    accepted = cand["metrics"]["overall"] >= self.min_overall and gain >= self.min_gain
                    ...
                    child = self.memory.blossoms.add_node({
                        ...
                        "scores": {"overall": cand["metrics"]["overall"], "metrics": cand["metrics"]},
                        "features": None,
                        "tags": ["expand", "sharpened"] + (["accepted"] if accepted else ["rejected"]),
                        "sharpen_passes": cand["iters"],
                        "sharpen_gain": gain,
                        "sharpen_meta": {
                            "iterations": cand["iterations"],
                            "hallucination_issues": cand["hallucination_issues"],
                    ...
                    try:
                        ...
                        self.logger.log("BlossomSharpenPersistWarn", {"error": str(e)})
            ...
            path_score = sum((n["scores"] or {}).get("overall", 0.0) for n in best_path) / max(1, len(best_path))

            # 5) Emit training events (aligns with your summarizer)
            try:
                title = doc.get("title", "")
                baseline_text = baseline_summary
                enhanced_text = best_node.sharpened_text or best_node.state_text or baseline_text
                baseline_metrics = base_metrics
                enhanced_metrics = (best_node.scores or {}).get("metrics", {"overall": best_overall})
            ...
            # 6) Finalize
            self.memory.blossoms.finalize(blossom.id, {
                "nodes": created_nodes,
                "edges": created_edges,
    ...
    def _expand_and_sharpen(self, doc: Dict[str, Any], baseline_text: str, k: int) -> List[Dict[str, Any]]:
        """
        ...
        diversity_prompt = self._build_diversity_prompt(title, abstract, baseline_text, k)
        diverse_seed = self.call_llm(diversity_prompt).strip()
        seeds = [s.strip("-• ").strip() for s in diverse_seed.split("\n") if s.strip()]
    ...
    def _build_diversity_prompt(self, title: str, abstract: str, summary: str, k: int) -> str:
...
""".strip()
    ...
    def _verify_hallucinations(self, summary: str, abstract: str, arxiv_summary: str):
    ...
    def _verify_figure_grounding(self, summary: str, paper_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for sent in sentences:
            m = re.findall(r"(\b\d+\.?\d*\s*(%|percent|accuracy|precision|recall|f1|auc|rmse|mae|bleu)\b)", sent, flags=re.I)
            if m:
                claims.append({
                    "claim": sent,
                    "value": m[0][0],
                    "metric": m[0][1],
                    "has_citation": any(t in sent.lower() for t in ["figure", "fig.", "table", "as shown", "see"])
                })
        cited = sum(1 for c in claims if c["has_citation"])
        rate = cited / max(1, len(claims))
        return {"total_claims": len(claims), "properly_cited": cited, "citation_rate": rate, "overall_figure_score": rate, "claims": claims}

    def _fetch_baseline_summary(self, doc_id, doc: Dict[str, Any]) -> Tuple[str, Optional[Any]]:
        """
        ...
        if baseline_obj:
            ...
            baseline_text = self._extract_summary(t) or (baseline_obj.meta or {}).get("summary", "")
    ...
    def _persist_sharpening_result(self, goal_id, run_id, doc, baseline_text: str, cand: Dict[str, Any]):
        ...
        """
        store_r = self.memory.sharpening_results 
        store_p = self.memory.sharpening_predictions
        title = doc.get("title", "")
        if store_r:
            store_r.insert({
                ...
                "preferred_output": cand["summary"] if cand["metrics"]["overall"] >= self.min_overall else baseline_text,
                "winner": "b" if cand["metrics"]["overall"] >= self.min_overall else "a",
                ...
                "score_diff": cand["metrics"]["overall"],  # baseline assumed 0 if unknown
