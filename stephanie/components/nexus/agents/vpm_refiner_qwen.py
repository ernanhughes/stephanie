...
class VPMRefinerConfig:
    ...
    operation_sequences: Dict[str, List[Dict]] = field(default_factory=lambda: {
        ...
        "debias": [
            ...
            {"type": "logic", "params": {"op": "SUB", "a": ("channel", 0), "b": ("map", "risk"), "dst": 0, "blend": 0.75}}
        ],
        "bridge_tame": [
...
class VPMRefinerAgent(BaseAgent):
    ...
    def _metric_bar(self, phi: Dict[str, float], utility: float, step_idx: int, W: int) -> np.ndarray:
        """
        Constructs a small HWC bar visualizing key metrics.
        W is the required width for concatenation alignment.
        """
        H = 32 # Height remains fixed
        # W is now the passed-in width from _compose_frame (e.g., 708)
        bar = np.zeros((H, W, 3), dtype=np.uint8)
        
        # ... (rest of your metric bar logic, including the Pillow text fix from before) ...
        
        # Ensure text_x calculation uses the correct, full width W
        ...
        text_x = W - text_w - 5 # Uses the correct W
        text_y = (H - text_h) // 2
    ...
    def _compose_frame(self, initial_rgb: np.ndarray, current_rgb: np.ndarray, utility: float, phi: Dict[str, float], step_idx: int) -> np.ndarray:
        ...
        # Get the height and width of the map visualizations for alignment
        H, W, _ = initial_rgb.shape # W is 708 according to the error

        # Create the metric bar
        metric_bar = self._metric_bar(phi, utility, step_idx, W)
