...
class AttentionConfig(BaseModel):
    ...
    min_attention: float = Field(0.1, ge=0.05, le=0.3, description="Minimum attention allocation")
    max_attention: float = Field(0.8, ge=0.6, le=0.95, description="Maximum attention allocation")
    learning_rate: float = Field(0.01, ge=0.001, le=0.05, description="Learning rate for attention updates")
    crisis_response_factor: float = Field(1.5, ge=1.2, le=2.0, description="Factor for crisis response")
...
class AttentionRecord:
    ...
    id: str = field(default_factory=lambda: f"attn_{int(time.time())}_{np.random.randint(1000)}")
...
class AttentionManager:
    ...
    def __init__(self, cfg: Dict[str, Any], initial_weights: Dict[str, float]):
        ...
        log.info("AttentionManager initialized with dynamic allocation features")
    ...
    def update_attention(
        ...
        try:
            ...
            self.attention_weights[layer] = max(
                ...
                min(self.config.max_attention, self.attention_weights[layer] + delta)
            )
            ...
            log.debug(f"Updated attention for {layer} (delta={delta:.3f}): {self.attention_weights}")
            return self.attention_weights.copy()
    ...
    def _validate_weights(self):
        ...
        for layer, weight in self.attention_weights.items():
            if weight < self.config.min_attention or weight > self.config.max_attention:
                log.warning(f"Attention weight for {layer} ({weight}) outside bounds, clamping")
                self.attention_weights[layer] = max(
                    self.config.min_attention,
        ...
        if abs(total - 1.0) > 0.01:
            log.warning(f"Attention weights don't sum to 1 ({total}), normalizing")
    ...
    def _update_metrics(self, old_weights: Dict[str, float], delta: float):
        ...
        changes = [abs(self.attention_weights[l] - old_weights[l]) for l in self.attention_weights]
        ...
        if len(self.attention_history) >= 10:
            recent_changes = [abs(self.attention_history[-i].attention_weights[l] - 
    ...
    def apply_crisis_response(self, crisis_level: float) -> Dict[str, float]:
        ...
        try:
            ...
            reptilian_delta = crisis_level * self.config.crisis_response_factor * 0.1
            self.attention_weights[AttentionLayer.REPTILIAN.value] = min(
                self.config.max_attention,
                self.attention_weights[AttentionLayer.REPTILIAN.value] + reptilian_delta
            )
            
            # Decrease primate attention (less abstract reasoning during crisis)
            primate_delta = -crisis_level * self.config.crisis_response_factor * 0.1
            self.attention_weights[AttentionLayer.PRIMATE.value] = max(
                self.config.min_attention,
                self.attention_weights[AttentionLayer.PRIMATE.value] + primate_delta
            )
            ...
            log.info(f"Applied crisis response (level={crisis_level:.3f}): {self.attention_weights}")
            return self.attention_weights.copy()
            
        except Exception as e:
            log.error(f"Error applying crisis response: {str(e)}", exc_info=True)
            return self.attention_weights.copy()
    ...
    def get_ssp_integration_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "attention_reptilian": self.attention_weights[AttentionLayer.REPTILIAN.value],
            "attention_mammalian": self.attention_weights[AttentionLayer.MAMMALIAN.value],
            "attention_primate": self.attention_weights[AttentionLayer.PRIMATE.value]
        }
    ...
    def evaluate_update_success(
        ...
        record = next((a for a in self.attention_history if a.id == attention_id), None)
        if not record:
        ...
        log.debug(f"Attention update evaluation: {attention_id} - success={success}, reward={reward:.3f}")
        return success
    ...
    def get_optimal_attention_profile(
        ...
        for record, weight in similar:
            ...
            for layer in avg_weights:
                avg_weights[layer] += record.attention_weights.get(layer, 0.0) * weight
