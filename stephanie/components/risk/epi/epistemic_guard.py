...
class GuardInput:
    ...
    def context_payload(self) -> Dict[str, Any]:
        return {"question": self.question, "reference": self.reference, **(self.meta or {})}
...
class HalVisModule:
    ...
    def detect_hallucination(self, reference: str, hypothesis: str) -> Dict[str, Any]:
        ...
        metrics = {"mean_energy": float(R.mean()), "max_energy": float(R.max()), "entropy": float(G.mean()), "tokens": int(n)}
...
class EGVisual:
    def __init__(self, out_dir: str, seed: int = 42):
        self.out_dir = out_dir; _ensure_dir(out_dir); self.seed = seed; self._umap = None
    def _project(self, vpm: np.ndarray) -> np.ndarray:
        if umap is None:
            x = vpm[:, :2]; x = (x - x.mean(0)) / (x.std(0) + 1e-8); return x
    ...
    def render(self, trace_id: str, vpm: np.ndarray):
        ...
        R = self._norm(vpm[:,0]); proj = self._project(vpm)
        if plt is None:
            if Image:
                for p in (field, strip, legend): Image.new("RGBA",(8,8),(0,0,0,0)).save(p)
            return field, strip, legend
        import matplotlib.pyplot as plt  # local alias
        plt.figure(figsize=(5.6,4.6))
        plt.scatter(proj[:,0], proj[:,1], c=R, s=10, cmap="inferno", alpha=0.95)
        ...
        plt.axis("off"); plt.text(0.02,0.98,"Channels:\nR=Δ-energy (hallucination)\nG=entropy/uncertainty\nB=HRM↔Tiny disagreement\nA=1−confidence",va="top",fontsize=8)
        plt.tight_layout(); plt.savefig(legend, dpi=200); plt.close()
        return field, strip, legend

class BadgeGenerator:
    def __init__(self, out_dir: str):
        self.out_dir = out_dir; _ensure_dir(out_dir)
    def render_badge(self, trace_id: str, risk: float, mean_energy: float, max_B: float, trust: float = 0.8, recency: float = 0.0) -> str:
        if Image is None:
        ...
        r,g,b = colorsys.hsv_to_rgb(hue, min(1.0, mean_energy*1.5), 0.9*(1-0.3*recency))
        base = Image.new("RGBA", (96,96), (int(r*255), int(g*255), int(b*255), int(255*(1-max_B))))
        draw = ImageDraw.Draw(base); halo = int(255 * min(1.0, mean_energy*0.7 + risk*0.3))
        ...
        path = os.path.join(self.out_dir, f"{trace_id}_badge.png"); base.save(path); return path

class EvidenceStore:
    def __init__(self, base_dir: str):
        self.base = base_dir; _ensure_dir(self.base)
    def save_vpm(self, trace_id: str, vpm: np.ndarray) -> str:
        p = os.path.join(self.base, f"{trace_id}_vpm.npz")
        np.savez_compressed(p, vpm=vpm); return p
    def store_evidence(self, data: Dict[str, Any]) -> str:
        eid = data.get("evidence_id") or str(uuid.uuid4()); p = os.path.join(self.base, f"{eid}.json")
...
class EpistemicGuard:
    ...
    """
    def __init__(self, root_out_dir: str = "./runs/risk_visuals", thresholds=(0.2,0.6), seed: int = 42):
    ...
    async def assess(self, data: GuardInput) -> GuardOutput:
        ...
        vpm = hv["vpm_tensor"].copy(); metrics = hv.get("metrics", {}); n_tokens = vpm.shape[0]
        disc = compute_disagreement(data.hrm_view, data.tiny_view, n_tokens); vpm[:,2] = disc["B"]
        ...
        evidence_id = self.store.store_evidence({
            "evidence_id": f"eg-{trace_id}", "schema": "eg.v1", "trace_id": trace_id, "created_at": _now_iso(),
            "risk": risk, "thresholds": {"low": low, "high": high},
            "metrics": {**metrics, "max_B": disc["max_B"], "disagree_rate": disc["disagree_rate"]},
            "vpm_path": vpm_path, "field_path": field_path, "strip_path": strip_path, "legend_path": legend_path,
        ...
        return GuardOutput(
            trace_id=trace_id, risk=risk, thresholds=(low, high), route=route,
            metrics={**metrics, "max_B": disc["max_B"], "disagree_rate": disc["disagree_rate"]},
            vpm_path=vpm_path, field_path=field_path, strip_path=strip_path, legend_path=legend_path,
            badge_path=badge_path, evidence_id=evidence_id,
        )
