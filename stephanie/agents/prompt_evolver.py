...
class EvaluatorAdapter:
    ...
    def __init__(self, evaluator, logger=None):
        ...
        self.logger = logger
        _logger.debug(f"EvaluatorAdapter initialized with evaluator: {type(evaluator).__name__}")
    ...
    def _extract_score(self, obj: Any) -> Optional[float]:
        ...
        try:
            ...
            _logger.warning(f"Failed to extract score from evaluator response: {e}, type: {type(obj)}")
        return None
    ...
    def score_prompt_only(self, prompt: str, goal: str = "", context: Dict[str, Any] | None = None) -> Optional[float]:
        ...
        _logger.debug(f"Scoring prompt-only (goal: {goal[:50]}..., prompt length: {len(prompt)})")
        if self.evaluator is None:
            _logger.warning("No evaluator available for scoring")
            return None
        try:
            if hasattr(self.evaluator, "score_single"):
                score = float(self.evaluator.score_single(prompt, goal=goal, context=context or {}))
                _logger.debug(f"score_single method returned: {score}")
                return score
            res = self.evaluator.evaluate({"prompt": prompt, "goal": goal}, context=context or {})
            score = self._extract_score(res)
            _logger.debug(f"evaluate method returned score: {score}")
            return score
        except Exception as e:
            _logger.error(f"Evaluator scoring failed: {e}")
            if self.logger:
    ...
    def score_against_reference(
        ...
        _logger.debug(f"Scoring against reference (ref length: {len(reference_output)}, prompt length: {len(prompt)})")
        if self.evaluator is None:
            _logger.warning("No evaluator available for reference scoring")
            return None
        try:
            if hasattr(self.evaluator, "score_single"):
                score = float(
                ...
                _logger.debug(f"score_single with reference returned: {score}")
                return score
            res = self.evaluator.evaluate(
                {"prompt": prompt, "reference_output": reference_output}, context=context or {}
            )
            score = self._extract_score(res)
            _logger.debug(f"evaluate with reference returned score: {score}")
            return score
        except Exception as e:
            _logger.error(f"Reference scoring failed: {e}")
            if self.logger:
...
class StrategyMutationPass:
    ...
    def __init__(self, score_fn, logger=None):
        ...
        self.logger = logger
        _logger.debug("StrategyMutationPass initialized")
    ...
    def apply(self, base_prompt: str, metadata: dict) -> list[dict]:
        ...
        """
        _logger.debug(f"Applying strategy mutations to prompt (length: {len(base_prompt)})")
        ...
        _logger.debug(f"Generated {len(candidates)} mutation candidates")

        for i, variant in enumerate(candidates):
            try:
                _logger.debug(f"Scoring mutation candidate {i+1}/{len(candidates)}")
                score = self.score_fn(variant, metadata) if self.score_fn else None
                _logger.debug(f"Mutation candidate {i+1} scored: {score}")
            except Exception as e:
                score = None
                _logger.warning(f"Failed to score mutation candidate {i+1}: {e}")
                if self.logger:
                    self.logger.log(
        ...
        _logger.debug(f"Strategy mutations completed. Best score: {sorted_mutations[0].get('score') if sorted_mutations else 'N/A'}")
...
class PromptEvolver:
    ...
    def __init__(self, llm, logger=None, use_strategy_mutation: bool = False, evaluator_cfg=None, memory=None):
        _logger.info(f"Initializing PromptEvolver (strategy_mutation: {use_strategy_mutation})")
        self.llm = llm
        self.logger = logger
        ...
        try:
            ...
            _logger.debug("DSPy configured successfully")
        except Exception as e:
            _logger.error(f"Failed to configure DSPy: {e}")
            if self.logger:
        ...
        self.evaluator = None
        if evaluator_cfg:
            _logger.debug("Loading evaluator from configuration")
        ...
        _logger.debug(f"Evaluator adapter initialized: {self.evaluator is not None}")
        ...
        if self.use_strategy_mutation:
            _logger.debug("Setting up strategy mutation pass")
    ...
    def evolve(self, examples: list[dict], context: dict | None = None, sample_size: int = 10) -> list[str]:
        ...
        """
        _logger.info(f"Starting prompt evolution with {len(examples)} examples, sample_size: {sample_size}")
        context = context or {}
        self.sources_map = {}
        
        if not examples:
            _logger.warning("No examples provided for evolution")
            return []

        # Build DSPy training set from examples
        train_items = examples[: max(1, int(sample_size))]
        _logger.debug(f"Building DSPy training set with {len(train_items)} items")
        ...
        _logger.debug(f"Training set built with {len(training_set)} examples")
        ...
        def eval_metric(example: Example, pred, trace=None):
            ...
            try:
                ...
                if not refined:
                    _logger.debug("Empty refined prompt in evaluation")
                    return 1.0
                    
                if self.adapter is None:
                    _logger.debug("No evaluator adapter available, using default score")
                    return 1.0
                ...
                _logger.debug(f"Evaluation metric score: {final_score}")
                return final_score
                
            except Exception as e:
                _logger.error(f"Evaluation metric failed: {e}")
                if self.logger:
                    self.logger.log("DSPyMetricFailed", {"error": str(e)})
                return 1.0

        # Compile DSPy program with BootstrapFewShot
        _logger.info("Compiling DSPy program with BootstrapFewShot")
        ...
        try:
            ...
            _logger.info("DSPy program compiled successfully")
        except Exception as e:
            _logger.error(f"DSPy compilation failed: {e}")
            return []

        # Generate refined prompts using the tuned program
        refined_prompts: List[str] = []
        _logger.info(f"Generating refined prompts for {len(examples)} examples")

        for i, ex in enumerate(examples):
            try:
                _logger.debug(f"Processing example {i+1}/{len(examples)}")
                ...
                if refined:
                    ...
                    _logger.debug(f"Generated DSPy refined prompt (length: {len(refined)})")
                else:
                    _logger.debug("No refined prompt generated for this example")
            except Exception as e:
                _logger.error(f"Failed to generate prompt for example {i+1}: {e}")
                if self.logger:
                    self.logger.log("DSPyPromptEvolutionFailed", {"error": str(e)})

        _logger.info(f"DSPy evolution generated {len(refined_prompts)} refined prompts")

        # Apply strategy mutations if enabled
        if self.use_strategy_mutation and self.strategy_pass is not None:
            _logger.info("Applying strategy mutations")
            strategy_count = 0
            
            for i, ex in enumerate(examples):
                base_prompt = ex.get("prompt_text", "")
                if not base_prompt:
                    _logger.debug(f"Skipping example {i+1} - no base prompt")
                    continue
                ...
                try:
                    _logger.debug(f"Generating strategy mutations for example {i+1}")
                    ...
                    for mut in mutations:
                        ...
                        if prompt_text:
                            ...
                            strategy_count += 1
                            _logger.debug(f"Added strategy mutation (score: {mut.get('score')})")
                except Exception as e:
                    _logger.error(f"Strategy mutation failed for example {i+1}: {e}")
                    if self.logger:
                        self.logger.log("StrategyMutationFailed", {"error": str(e)})
            
            _logger.info(f"Strategy mutations generated {strategy_count} additional prompts")

        # Deduplicate while preserving order
        seen = set()
        deduped_prompts = [p for p in refined_prompts if p and not (p in seen or seen.add(p))]
        _logger.info(f"Evolution complete. Total unique prompts: {len(deduped_prompts)} "
    ...
    def score_prompt(self, prompt: str, reference_output: str = "", context: dict | None = None) -> float:
        ...
        _logger.debug(f"Scoring prompt against reference (lengths: prompt={len(prompt)}, ref={len(reference_output)})")
        s = self.adapter.score_against_reference(prompt, reference_output, context or {})
        score = float(s) if s is not None else -1.0
        _logger.debug(f"Prompt score against reference: {score}")
        return score

    def score_prompt_only(self, prompt: str, goal: str = "", context: dict | None = None) -> float:
        """Score prompt against goal only."""
        _logger.debug(f"Scoring prompt against goal (lengths: prompt={len(prompt)}, goal={len(goal)})")
        s = self.adapter.score_prompt_only(prompt, goal, context or {})
        score = float(s) if s is not None else -1.0
        _logger.debug(f"Prompt score against goal: {score}")
        return score
...
class PromptEvolverAgent(BaseAgent):
    ...
    def __init__(self, cfg: Dict[str, Any], memory, container, logger):
        _logger.info("Initializing PromptEvolverAgent")
        ...
        _logger.info(f"PromptEvolverAgent configured: "
    ...
    def _safe_get_service(self, key: str):
        ...
        try:
            ...
            _logger.debug(f"Successfully retrieved service: {key}")
            return service
        except Exception as e:
            _logger.warning(f"Service '{key}' not available: {e}")
    ...
    async def run(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        _logger.info(f"Processing {len(examples)} examples with configuration: "
        ...
        _logger.debug("Initializing PromptEvolver")
        ...
        try:
            ...
            _logger.info(f"Prompt evolution completed. Generated {len(refined_prompts)} refined prompts")
        except asyncio.TimeoutError:
            _logger.error(f"Prompt evolution timed out after {self.timeout} seconds")
            self._log_err("EvolutionTimedOut", {"timeout": self.timeout})
            refined_prompts = []
        except Exception as e:
            _logger.error(f"Prompt evolution failed: {e}")
        ...
        _logger.debug(f"After deduplication: {len(refined_prompts)} prompts")
        ...
        if include_original:
            _logger.debug("Including original prompts in results")
            for p in original_prompts:
                if p not in seen:
                    refined_prompts.insert(0, p)
                    seen.add(p)
            _logger.debug(f"After including originals: {len(refined_prompts)} total prompts")
        ...
        _logger.info(f"Scoring {len(refined_prompts)} candidates (scoring_enabled: {scoring_enabled})")
        ...
        for idx, prompt in enumerate(refined_prompts):
            try:
                ...
                if scoring_enabled:
                    if goal_text:
                        _logger.debug(f"Scoring candidate {idx+1} against goal")
                        score = float(evolver.score_prompt_only(prompt, goal=goal_text, context=context))
                    else:
                        _logger.debug(f"Scoring candidate {idx+1} against reference")
                ...
                _logger.debug(f"Candidate {idx+1}: source={source}, score={score}, "
                ...
                if timeline_opened:
                    try:
                        ...
                        _logger.debug(f"Added candidate {idx+1} to timeline")
                    except Exception as te:
                        _logger.warning(f"Failed to add candidate to timeline: {te}")
                        self._log_warn("TimelineNodeFailed", {"error": str(te)})
                        
            except Exception as e:
                _logger.error(f"Failed to process candidate {idx+1}: {e}")
                self._log_warn("CandidateBuildFailed", {"error": str(e)})

        # Select best prompt based on scores
        _logger.info("Selecting best prompt from candidates")
        ...
        if best_prompt is None and original_prompts:
            _logger.warning("No refined prompts available, falling back to original prompt")
            best_prompt, best_score, best_source = original_prompts[0], None, "fallback_original"

        _logger.info(f"Best prompt selected: source={best_source}, score={best_score}, "
        ...
        if timeline_opened:
            try:
                _logger.debug("Finalizing timeline")
                ...
                _logger.info(f"Timeline finalized at: {timeline_path}")
            except Exception as e:
                _logger.error(f"Failed to finalize timeline: {e}")
        ...
        _logger.info(f"Evolution completed: {len(candidates)} candidates, "
        ...
        _logger.info("PromptEvolverAgent run completed successfully")
        return context
    ...
    def _pick_best_with_source(
        ...
        """
        _logger.debug(f"Selecting best prompt from {len(candidates)} candidates")
        if not candidates:
            _logger.warning("No candidates available for selection")
        ...
        if scored:
            ...
            _logger.debug(f"Best candidate selected with score: {best['score']}")
            return best["prompt"], float(best["score"]), best.get("source")
        else:
            # Fallback to first candidate if no scores available
            c = candidates[0]
            _logger.debug("No scored candidates available, using first candidate")
    ...
    async def _emit_event(self, event: str, payload: Dict[str, Any]) -> None:
        ...
        if not self.memory or not getattr(self.memory, "bus", None):
            _logger.debug(f"No event bus available, skipping event: {event}")
            return
        ...
        try:
            ...
            _logger.debug(f"Event published: {event}")
        except Exception as e:
            _logger.warning(f"Failed to publish event '{event}': {e}")
