...
def _log_state(prefix: str, state: Strategy):
    log.debug("[%s] Strategy v%s | skeptic=%.2f editor=%.2f risk=%.2f threshold=%.2f",
...
class StrategyManager:
    ...
    def _load_or_default(self) -> Strategy:
        try:
            ...
            if not snap or not snap.get("payload"):
                log.debug("[StrategyManager] No snapshot found, using defaults")
                return Strategy()
            s = Strategy.from_dict(snap["payload"])
            _log_state("Loaded snapshot", s)
            return s
        except Exception as e:
            log.warning("[StrategyManager] Failed to load snapshot: %s", e)
            return Strategy()
    ...
    def _propose(self, avg_gain: float) -> Strategy:
        ...
        """
        log.debug("[StrategyManager] Proposing new strategy (avg_gain=%.4f)", avg_gain)
        s = self.state
    ...
    def evolve(self, iterations: List[Dict[str, Any]], context: Optional[Dict[str, Any]]):
        ...
        log.debug("[StrategyManager] Assigned variant=%s case_id=%s exp_id=%s",
    ...
    def track_section(self, case, iterations: List[Dict[str, Any]], context: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            log.debug("[StrategyManager] track_section case_id=%s run_id=%s "
    ...
    def validate_ab(self, context: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            log.debug("[StrategyManager] validate_ab exp_id=%s stats=%s", exp.id, stats)
    ...
    def maybe_commit_strategy(self, validation: Optional[Dict[str, Any]], context: Dict[str, Any]) -> bool:
        ...
        log.debug("[StrategyManager] maybe_commit_strategy called delta=%.4f rel=%.4f p_ok=%s",
             delta, rel, p_ok)
