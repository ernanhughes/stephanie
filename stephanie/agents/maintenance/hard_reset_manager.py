...
class HardResetManager(BaseAgent):
    ...
    def _restore_backup(self):
        ...
        self.logger.log(
            "HardResetRestore", {"from": self.backup_dir, "to": self.model_dir}
        )
    ...
    def create_backup(self):
        ...
        self.logger.log(
            "HardResetBackupCreated",
            {"backup_id": backup_id, "model_versions": metadata["model_versions"]},
        )
    ...
    def _get_current_versions(self):
        ...
        results = self.memory.session.execute(query).fetchall()
        return {
            f"{r.model_type}/{r.target_type}/{r.dimension}": r.version for r in results
        }
    ...
    def _alignment_drift(self, current_score):
        ...
        if current_score < historical * 0.7:  # 30% drop
            self.logger.log(
                "AlignmentDriftDetected",
                {"current_score": current_score, "historical_avg": historical},
            )
            return True
        return False
    ...
    def _score_volatility(self):
        ...
        for r in results:
            if r.volatility > self.reset_thresholds.get("volatility", 0.5):
                self.logger.log(
                    "ScoreVolatilityDetected",
                    {"dimension": r.dimension, "volatility": r.volatility},
                )
                return True
        return False
    ...
    def check_for_reset(self, dry_run=False):
        ...
        if self._ethics_failure(recent_scores) or self._instability_detected(
            recent_scores
        ):
            self.logger.log(
                "HardResetTriggered",
