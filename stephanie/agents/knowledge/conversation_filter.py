...
class ConversationFilterAgent(BaseAgent):
    def __init__(
        self, cfg: Dict[str, Any], memory: Any, container: Any, logger
    ):
        super().__init__(cfg, memory, container, logger)
        self.kfg = CFConfig(**cfg.get("conversation_filter", {}))
        self.goal_scorer = GoalScorer(logger=logger)
        self.logger.info(
            "ConversationFilterAgent initialized", {"config": asdict(self.kfg)}
        )
    ...
    async def run(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        chat = [
            m for m in chat if m.get("text") and len(m["text"].strip()) > 5
        ]
        ...
        if self.kfg.use_embeddings:
            try:
                ...
                self.logger.log(
                    "SectionEmbeddingFailed",
                    {"error": str(e), "section": section_name},
                )

        try:
            scored = self._score_messages(
                chat, section_text, sec_emb, goal_template
            )
            ...
            thr = self._dynamic_threshold(
                [m["score"] for m in scored], self.kfg.min_keep
            )
                    ...
                    "top_samples": [
                        {
                            ...
                            "sim": round(m.get("similarity", 0.0), 3),
                            "text": m["text"][:120],
                        }
    ...
    def _score_messages(
        ...
        for start in range(0, len(chat), self.kfg.window_stride):
            ...
            window_scored = self._score_window(
                window, section_text, sec_emb, goal_template
            )
    ...
    def _score_window(
        ...
        for msg in chat:
            ...
            vpm_score = self.goal_scorer.score(
                kind="text", goal=goal_template, vpm_row=dims
            ).get("score", 0.0)
    ...
    def _embedding_similarity(
        self, text: str, sec_emb: np.ndarray, section_text: str
    ) -> float:
        try:
            if len(text) > 50:
    ...
    def _vpm_dims(self, text: str, section_text: str) -> Dict[str, float]:
        ...
        evidence = (
            ...
            else min(1.0, 1.2 * lexical_overlap(text, section_text))
        )
    ...
    def _extra_score(self, text: str, dims: Dict[str, float]) -> float:
        ...
        if len(text) < self.kfg.length_cutoff or t in {
            "ok",
            "thanks",
            "got it",
            "yup",
    ...
    def _reason_from_dims(self, dims: Dict[str, float], extra: float) -> str:
        bits = []
        if dims["correctness"] > 0.7:
            bits.append("factually accurate")
        if dims["progress"] > 0.7:
            bits.append("advances understanding")
        if dims["evidence"] > 0.7:
            bits.append("well-supported")
        if dims["novelty"] > 0.7:
            bits.append("provides new insight")
        if extra > 0.05:
            bits.append("strong evidence refs")
        if extra < -0.05:
            bits.append("likely chatter")
    ...
    def _apply_domain_rules(
        self, scored: List[Dict[str, Any]], domain: str
    ) -> List[Dict[str, Any]]:
        changed = 0
        for m in scored:
            before = m["score"]
            ...
            if "machine learning" in domain:
                ...
                if any(
                    t in tl for t in ["proof", "theorem", "lemma", "corollary"]
                ):
        ...
        if changed:
            self.logger.log(
                "DomainRulesApplied", {"domain": domain, "adjusted": changed}
            )
        return scored
    ...
    def _critical_path(
        ...
        ranked = sorted(
            msgs, key=lambda x: (-x["score"], x.get("timestamp", 0))
        )
        path = [ranked[0]]
