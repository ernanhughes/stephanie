...
class PolicyAnalysisAgent(BaseAgent):
    def __init__(self, cfg, memory, container, logger):
        ...
        self.dimensions = cfg.get(
            "dimensions", ["alignment", "clarity", "novelty"]
        )
    ...
    async def run(self, context: dict) -> dict:
        ...
        for dim in self.dimensions:
            report = self.analyzer.generate_policy_report(
                dim, pipeline_run_id=pipeline_run_id
            )
            reports[dim] = report
            ...
            for insight in report.get("insights", []):
                self.logger.log(
                    "PolicyInsight", {"dimension": dim, "insight": insight}
                )
            ...
            self.logger.log(
                "PolicyReportSaved", {"dimension": dim, "path": file_path}
            )
...
class PolicyAnalyzer:
    ...
    def analyze_dimension(
        self, dimension: str, pipeline_run_id: int = None
    ) -> Dict[str, Any]:
        try:
    ...
    def _get_sicql_data(
        ...
        if pipeline_run_id:
            query = query.filter(
                EvaluationORM.pipeline_run_id == pipeline_run_id
            )
    ...
    def _get_mrq_data(
        ...
        return [
            self._format_score(score, eval_obj) for score, eval_obj in results
        ]
    ...
    def _analyze_policy_patterns(
        self, sicql_data: List[Dict]
    ) -> Dict[str, Any]:
        """Analyze policy patterns across samples"""
        valid_logits = [
            d["policy_logits"]
            for d in sicql_data
            if d["policy_logits"] is not None and len(d["policy_logits"]) > 1
        ]
        ...
        action_counts = np.bincount(
            most_probable_actions, minlength=action_probs.shape[1]
        )
    ...
    def _calculate_policy_consistency(
        self, sicql_data: List[Dict]
    ) -> Dict[str, Any]:
        ...
        actions = np.argmax(
            [
                d["policy_logits"]
                for d in sicql_data
                if d["policy_logits"] is not None
            ],
            axis=1,
        )
        policy_changes = (
            np.sum(actions[1:] != actions[:-1]) / len(actions)
            if len(actions) > 1
            else 0
        )
        ...
        q_values = np.array(
            [d["q_value"] for d in sicql_data if d["q_value"] is not None]
        )
        v_values = np.array(
            [d["v_value"] for d in sicql_data if d["v_value"] is not None]
        )
        ...
        return {
            ...
            "q_v_correlation": float(q_v_correlation)
            if q_v_correlation is not None
            else None,
            "sample_count": len(sicql_data),
        }

    def _compare_with_source(
        self, sicql_data: List[Dict], reference_data: List[Dict], label: str
    ) -> Dict[str, Any]:
        ...
        if len(sicql_scores) < 5:
            log.warning(
                """Low sample count for correlation
                source: %s
                sample_count: %d
                dimension: %s
            """,
                label,
                len(sicql_scores),
                sicql_data[0].get("dimension", "unknown"),
            )
        ...
        score_deviation = (
            np.mean([abs(a - b) for a, b in zip(sicql_scores, ref_scores)])
            if sicql_scores
            else None
        )
        ...
        return {
            ...
            if score_deviation is not None
            else None,
    ...
    def _find_high_uncertainty(self, sicql_data: List[Dict]) -> List[Dict]:
        ...
        return [
            ...
            for d in sicql_data
            if d["uncertainty"]
            and d["uncertainty"] > self.uncertainty_threshold
        ]
    ...
    def _calculate_entropy(self, sicql_data: List[Dict]) -> Dict[str, float]:
        ...
        action_probs = np.exp(valid_logits) / np.exp(valid_logits).sum(
        ...
        return {
            ...
            "high_entropy": int(
                np.sum(entropy > np.median(entropy) + np.std(entropy))
            ),
        }
    ...
    def _detect_policy_drift(self, sicql_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(unique_actions) < 3:
                ...
                cluster_changes = np.sum(clusters[1:] != clusters[:-1]) / len(
                    clusters
                )
        except Exception:
            cluster_changes = 0
    ...
    def generate_policy_report(
        ...
        analysis = self.analyze_dimension(
            dimension, pipeline_run_id=pipeline_run_id
        )
        ...
        report = {
            ...
            "policy_stability": policy_consistency.get(
            ...
            "action_drift_rate": policy_consistency.get(
                "action_drift_rate", 0.0
            ),
            **{
                f"{src}_correlation": comparison_dict.get(src, {}).get(
                    "score_correlation"
                )
                for src in ["mrq", "svm", "ebt", "llm"]
            },
            **{
                f"{src}_deviation": comparison_dict.get(src, {}).get(
                    "score_deviation"
                )
                for src in ["mrq", "svm", "ebt", "llm"]
            },
            **{
                f"{src}_samples": comparison_dict.get(src, {}).get(
                    "sample_count"
                )
                for src in ["mrq", "svm", "ebt", "llm"]
            },
    ...
    def _generate_insights(self, stats: Dict) -> List[str]:
        ...
        if (
            stats.get("qv_correlation") is not None
            and stats["qv_correlation"] < 0.3
        ):
            insights.append(
                "⚠️ Weak alignment between Q and V — the policy may lack internal consistency."
            )
        ...
        if uncertainty_count > 50:
            insights.append(
                "⚠️ Model is highly uncertain — consider revisiting training or reward signal."
            )

        # Low policy stability
        if stats.get("policy_stability", 1.0) < 0.5:
            insights.append(
                "⚠️ Policy behavior is unstable — high action variance detected."
            )
    ...
    def visualize_policy(
        self, dimension: str, output_path: str = "logs/policy_visualization"
    ):
        """Generate policy visualization for a dimension"""
        try:
            sicql_data = self._get_sicql_data(dimension)
            if not sicql_data or not any(
                d["policy_logits"] for d in sicql_data
            ):
                return None
            ...
            sns.lineplot(
                data=df, x="timestamp", y="uncertainty", label="Uncertainty"
            )

            # Highlight high-uncertainty periods
            high_uncertainty = df[
                df["uncertainty"] > self.uncertainty_threshold
            ]
            for _, row in high_uncertainty.iterrows():
                plt.axvspan(
                    row["timestamp"], row["timestamp"], alpha=0.2, color="red"
                )
            ...
            sns.histplot(
                df["action"], bins=np.arange(-0.5, 3.5, 1), discrete=True
            )
    ...
    def generate_markdown_summary(self, analysis: Dict[str, Any]) -> str:
        lines = [
            f"# Policy Report for Dimension: **{analysis['dimension']}**\n"
        ]

        lines.append("## Summary Metrics:")
        lines.append(
            f"- Policy Stability: `{analysis.get('policy_stability')}`"
        )
        lines.append(f"- Q/V Correlation: `{analysis.get('q_v_correlation')}`")
        lines.append(
            f"- Score Correlation (vs MRQ): `{analysis.get('score_correlation')}`"
        )
        lines.append(f"- Score Deviation: `{analysis.get('score_deviation')}`")
        lines.append(
            f"- Entropy (avg): `{analysis.get('policy_entropy_avg')}`"
        )
        lines.append(
            f"- Uncertainty Cases: `{analysis.get('uncertainty_count')}`"
        )
        lines.append(
            f"- Policy Drift Detected: `{analysis.get('policy_drift')}`"
        )
        lines.append(
            f"- Action Drift Rate: `{analysis.get('action_drift_rate')}`"
        )
