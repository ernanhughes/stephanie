...
class CompilerAgent(ScoringMixin, BaseAgent):
    def __init__(self, cfg, memory=None, logger=None, full_cfg=None):
        ...
        self.executor = NodeExecutor(
            cfg, memory=memory, logger=logger, pipeline_runner=runner, tree=self.tree
        )
        ...
        if self.logger:
            self.logger.log(
                "CompilerAgentInit",
                {"message": "Initialized CompilerAgent", "config": cfg},
            )

    async def run(self, context: dict) -> dict:
        goal_text = context.get("goal", {}).get("goal_text", "No goal text provided")
        root_id = self.tree.add_root(
            goal_text, "Start solving this problem.", "Generate initial plan"
        )
        node = self.tree.nodes[root_id]

        if self.logger:
            self.logger.log(
                "CompilerStart", {"goal_text": goal_text, "root_id": root_id}
            )

        for iteration in range(5):  # Max iterations
            if self.logger:
                self.logger.log(
                    "CompilerIterationStart",
                    {
                        "iteration": iteration,
                        "node_id": node.id,
            ...
            if self.logger:
                self.logger.log(
                    "StepSelection",
                    {"node_id": node.id, "next_step_count": len(next_steps)},
                )
        ...
        if self.logger:
            self.logger.log(
                "FinalPromptBuilt",
                {"final_prompt": final_prompt, "path_length": len(best_path)},
            )
