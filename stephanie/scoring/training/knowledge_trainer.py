...
class KnowledgeTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dl):
        ...
        for G, A, B, AUXA, AUXB in dl:
            ...
            torch.nn.utils.clip_grad_norm_(
                self.model.encoder.parameters(), 0.5
            )
            torch.nn.utils.clip_grad_norm_(
                self.model.predictor.parameters(), 0.5
            )
            torch.nn.utils.clip_grad_norm_(
                self.model.aux_proj.parameters(), 0.5
            )
            opt.step()
    ...
    def train(self, pairs, dimension: str = "knowledge"):
        ...
        for epoch in range(self.epochs):
            ...
            self.logger.log(
                "KnowledgeTrainingEpoch", {"epoch": epoch + 1, "loss": loss}
            )
        ...
        )  # reuse q_head_file for aux_proj
        ...
        self.memory.training_stats.add_from_result(
            stats={},  # you can pass partials; losses can be None at start
