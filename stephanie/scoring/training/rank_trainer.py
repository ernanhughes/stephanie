# stephanie/scoring/training/rank_trainer.py
...
class ContrastiveRankerTrainer(BaseTrainer):
    ...
    def train(self, samples, dimension):
        ...
        pbar = self._create_progress_bar(
            samples, f"Processing {dimension} pairs", total=len(samples)
        )
        ...
        for pair in pbar:
            ...
            if "value_a" in pair:
                absolute_scores.append(
                    (pair["output_a"], pair["title"], pair["value_a"])
                )
            if "value_b" in pair:
                absolute_scores.append(
                    (pair["output_b"], pair["title"], pair["value_b"])
                )
        ...
        model = PreferenceRanker(
            embedding_dim=input_dim, hidden_dim=self.hidden_dim
        )
        ...
        dataset = torch.utils.data.TensorDataset(
            X_a_tensor, X_b_tensor, y_tensor
        )
        ...
        self.log_event(
            "ContrastiveRankerTrainingComplete",
            {
                "dimension": dimension,
                "pairs": len(samples),
                "calibration_samples": len(absolute_scores),
