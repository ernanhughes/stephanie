...
# --------- Simple logger stub (replace with your logger) ----------
class _Log:
    @staticmethod
    def info(msg, *a, **k):  print("[INFO]", msg)
    @staticmethod
    def warn(msg, *a, **k):  print("[WARN]", msg)
    @staticmethod
    def error(msg, *a, **k): print("[ERROR]", msg)
log = _Log()
...
# --------- MemCube client stub (replace with real) ----------
class MemCubeClient:
    """
    Minimal async stub. Replace with your production client.
    Expected methods:
      - query_calibration(kind, filters, sort, limit)
      - store_calibration(kind, payload)
      - guess_domain(question) -> str
    """
    def __init__(self):
        # in-memory calibration cache: { (kind, domain): record }
        self._calib: Dict[Tuple[str,str], Dict[str,Any]] = {}

    async def query_calibration(self, kind: str, filters: Dict[str,Any], sort=None, limit=1):
        dom = filters.get("domain", "general")
        rec = self._calib.get((kind, dom))
        await asyncio.sleep(0)  # yield
        return rec or None

    async def store_calibration(self, kind: str, payload: Dict[str,Any]):
        dom = payload.get("domain", "general")
        self._calib[(kind, dom)] = payload
        await asyncio.sleep(0)
        return True

    async def guess_domain(self, question: str) -> str:
        q = (question or "").lower()
        # toy heuristic; replace with your classifier
        if any(k in q for k in ["war","empire","treaty","century","king"]): return "history"
        if any(k in q for k in ["river","border","capital","country","latitude","longitude"]): return "geography"
        if any(k in q for k in ["cell","atom","neuron","quantum","protein"]): return "science"
        if any(k in q for k in ["api","database","algorithm","model","gpu","cloud"]): return "tech"
        return "general"
...
class RiskFeaturizer:
    ...
    async def featurize(self, question: str, context: str) -> Dict[str, float]:
        ...
        feats = {
            ...
            "ner_count": float(sum(w.istitle() for w in q_tokens)),  # toy NER proxy
...
class RiskModelBundle:
    ...
    def load(cls, path: str) -> "RiskModelBundle":
        ...
        if hasattr(obj, "predict_proba") and hasattr(obj, "feature_names"):
            return cls(clf=obj, feature_names=list(obj.feature_names), version=getattr(obj, "version", "risk-bundle.v1"))
        raise ValueError("Unsupported risk bundle format; expect dict with keys ['clf','feature_names']")
...
class DomainCalibratedRiskPredictor:
    ...
    def __init__(
        ...
        self.default_domains = default_domains or ["science","history","geography","tech","general"]
        self.default_thresholds = (float(default_thresholds[0]), float(default_thresholds[1]))
        ...
        if self.bundle is not None:
            missing = [k for k in self.featurizer.feature_order if k not in self.bundle.feature_names]
            if missing:
                log.warn(f"Risk bundle missing features used by featurizer: {missing}")
    ...
    def _vectorize(self, feats: Dict[str,float]) -> np.ndarray:
        ...
        """
        names = self.bundle.feature_names if self.bundle else self.featurizer.feature_order
        x = np.array([[float(feats.get(k, 0.0)) for k in names]], dtype=np.float32)
        return x, names
    ...
    def _predict_proba(self, x: np.ndarray) -> float:
        ...
        q_len_idx = (self.featurizer.feature_order.index("q_len")
                     if "q_len" in self.featurizer.feature_order else 0)
        gap_idx   = (self.featurizer.feature_order.index("coverage_gap")
                     if "coverage_gap" in self.featurizer.feature_order else 0)
        q_len = x[0, q_len_idx]
        gap   = x[0, gap_idx]
    ...
    async def explain(self, question: str, context: str) -> Dict[str,Any]:
        ...
        try:
            ...
            return {"ok": True, "expected_value": float(explainer.expected_value[0] if isinstance(explainer.expected_value, (list, tuple, np.ndarray)) else explainer.expected_value),
                    "shap_values": values}
