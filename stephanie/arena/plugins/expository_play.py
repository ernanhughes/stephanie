...
from stephanie.arena.plugins.interfaces import JobCtx, Play, PlayResult
...
class ExpositoryPlay:
    ...
    def __init__(self, session_maker, logger, k=12, target_words=700,
                 min_exp=0.45, min_blog=0.50):
        self.sm = session_maker
        self.log = logger
        self.k = k; self.target_words = target_words
        self.min_exp = min_exp; self.min_blog = min_blog

    def run(self, ctx: JobCtx) -> PlayResult:
        # rank & buffer
        buf = ExpositoryBufferStore(self.sm, self.log).rank_and_create_buffer(
            topic=ctx.topic, k=self.k, min_exp=self.min_exp, min_blog=self.min_blog
        )
        # stitch → draft
        from stephanie.agents.expository.stitcher import BlogStitcher
        ...
        draft = draft if isinstance(draft, object) else draft  # noop for illustration

        # store-level internal eval
        r = BlogDraftStore(self.sm, self.log).evaluate_and_mark(
            draft.id, min_readability=45, min_adjacent_coherence=0.62,
            keep_threshold=0.6, repetition_penalty_weight=0.1
        )
