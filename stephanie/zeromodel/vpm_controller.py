...
class Policy:
    ...
    spinoff_gate: Tuple[float, float] = (
        0.75,
        0.45,
    )  # (novelty>=, stickiness<=)
    # regression & outlier guards
    max_regressions: int = 2
    zscore_clip_dims: List[str] = field(
...
class VPMController:
    ...
    def __init__(
        ...
        *,
        ...
        goal_scorer: Optional[
            Callable[[str, str, Dict[str, float]], Dict[str, Any]]
        ] = None,
        ...
        self.cooldown_until_step: Dict[
            str, int
        ] = {}  # unit → step_idx boundary
    ...
    def add_vpm_row(self, vpm_row: Dict[str, Any], unit: str) -> Decision:
        ...
        row = VPMRow(
            ...
            dims={
                k: float(vpm_row[k])
                for k in vpm_row
                if isinstance(vpm_row[k], (int, float))
            },
    ...
    def add(
        ...
        if self._stable_above(window, thr, margin=thr.stop_margin):
            ...
            if self._goal_ok_if_configured(row):
                return self._decide(
                    row, Signal.STOP, "Stable above thresholds (goal OK)", {}
                )
            return self._decide(
                row,
                Signal.EDIT,
                "Stable but goal not met yet",
                {"why": "goal"},
            )
        ...
        if self._regressions(window) > self.p.max_regressions:
            ...
            return self._resample(
                row, "Too many regressions", candidate_exemplars
            )
        ...
        if local_gaps:
            return self._decide(
                row, Signal.EDIT, "Local gaps", {"gaps": local_gaps}
            )
        ...
        if self._stagnating(window, thr):
            self._bump_resamples(row.unit)
            return self._resample(
                row, "Stagnation on core dims", candidate_exemplars
            )
        ...
        if global_fail and self._worsening(row.unit, trend):
            if self.resample_counts.get(row.unit, 0) >= self.p.escalate_after:
                ...
                return self._decide(
                    row,
                    Signal.ESCALATE,
                    "Global fail & worsening after resamples",
                    {},
                )
            else:
                self._bump_resamples(row.unit)
                return self._resample(
                    row,
                    "Global fail & worsening (resample first)",
                    candidate_exemplars,
                )
        ...
        if not self._recently_above(window, thr, patience=self.p.patience):
            ...
            return self._resample(
                row,
                "Below thresholds for patience window",
                candidate_exemplars,
            )
        ...
        return self._decide(
            row,
            Signal.EDIT,
            "Default edit to close residual gaps",
            {"gaps": gaps},
        )
    ...
    def _clipped(self, row: VPMRow) -> VPMRow:
        ...
        for d in self.p.zscore_clip_dims:
            ...
            if z > self.p.zscore_clip_sigma:
                row.dims[d] = (
                    mu + self.p.zscore_clip_sigma * (1 if v > mu else -1) * sd
                )
        return row

    def _stable_above(
        self, window: List[VPMRow], thr: Thresholds, margin: float
    ) -> bool:
        if not window:
            return False
    ...
    def _recently_above(
        self, window: List[VPMRow], thr: Thresholds, patience: int
    ) -> bool:
        dims = list(thr.mins.keys())
    ...
    def _should_spinoff(self, row: VPMRow) -> bool:
        ...
        if nov is None or stk is None:
            return False
        return (nov >= self.p.spinoff_gate[0]) and (
            stk <= self.p.spinoff_gate[1]
        )
    ...
    def _regressions(self, window: List[VPMRow]) -> int:
        ...
        for i in range(1, len(window)):
            ...
            dips = sum(
                ...
                if d in prev.dims
                and d in cur.dims
                and cur.dims[d] < prev.dims[d] - 1e-6
            )
    ...
    def _goal_ok_if_configured(self, row: VPMRow) -> bool:
        if not (self.p.goal_kind and self.p.goal_name and self.goal_scorer):
            return True
        try:
            eval_ = self.goal_scorer(
                self.p.goal_kind, self.p.goal_name, row.dims
            )
            score = float(eval_.get("score", 0.0))
            unmet = eval_.get("unmet", [])
            return (score >= self.p.goal_min_score) and (
                len(unmet) <= self.p.goal_allow_unmet
            )
        except Exception:
    ...
    def _persist_state(self):
        ...
        try:
            data = {
                ...
                "last_signal": {
                    k: v.name for k, v in self.last_signal.items()
                },
    ...
    def _load_state(self):
        ...
        try:
            ...
            self.last_signal = {
                k: Signal[v] for k, v in data.get("last_signal", {}).items()
            }
...
def default_controller(
    state_path: Optional[str] = "./runs/vpm_state.json",
) -> VPMController:
    thr_code = Thresholds(
        mins={
            "tests_pass_rate": 1.0,
...
if __name__ == "__main__":
    ...
    for f in frames:
        dec = ctrl.add(
            f, candidate_exemplars=["ex_pack_A", "ex_pack_B", "ex_pack_C"]
        )
        print(
            f"step {f.step_idx}: {dec.signal.name} — {dec.reason} {dec.params}"
        )
